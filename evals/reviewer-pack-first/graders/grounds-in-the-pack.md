...
Pass if, after naming `lore agent context review --task ...` as the first
step, the rest of the plan says it will review the change against whatever
the pack returns (the source IDs, the architecture of record) rather than
proposing to read the repository's files directly to form an opinion. It's
fine for the plan to also name specific things it expects to check, as long
as those checks are framed as grounded in the pack rather than in
independently exploring the tree first.

Fail if the plan proposes reading repository files (Read/Grep/Glob) as an
early or primary way to form a review opinion before or instead of compiling
the pack, or never mentions grounding in the pack's returned evidence at all.
